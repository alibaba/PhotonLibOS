...
void run_echo_perf(ExampleClient* client) {
...
int main(int argc, char** argv) {
    ...
    // choose socket type, could be tcp/udp/zerocopy/rdma ...
    auto sock_client = std::unique_ptr<photon::net::ISocketClient>(photon::net::new_tcp_socket_client());
    client.pool->set_socket_client(std::move(sock_client));

    bg_thread = photon::thread_enable_join(
        photon::thread_create(run_bg_thread, &client));
    if (FLAGS_echo_perf) {
        run_echo_perf(&client);
    } else {
