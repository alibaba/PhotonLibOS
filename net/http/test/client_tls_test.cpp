...
TEST(client_proxy, connect_tunnel) {
    ...
    DEFER({ delete proxy_tcp; delete proxy_http; });
...
static void teardown_tunnel_test(TunnelTestEnv& env) {
    delete env.proxy_tcp;   // stops server, joins connection threads
    delete env.proxy_http;  // safe: no threads reference handler now
    delete env.tls_svr;
