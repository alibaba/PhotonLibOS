...
protected:
    void SetUp() override {
        GTEST_ASSERT_EQ(0, photon::init());
#ifdef PHOTON_UT_ENABLE_URING
        engine = photon::new_iouring_cascading_engine();
#else
...
TEST_F(event_engine, cascading_remove) {
    ...
    num_events = engine->wait_for_events(data, 5, -1UL);
    auto end = std::chrono::system_clock::now();
    ASSERT_EQ(0, num_events);

    // wait_for_events should return immediately, because the only event is removed
    auto time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ASSERT_TRUE(time_ms < 100);
