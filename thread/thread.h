...
namespace photon
{
    ...
    public:
        ...
        void signal() {
            if (_signaled.test_and_set(std::memory_order_acquire))
                thread_interrupt(_waiter, -1); // wake up the waiter when signled twice
        }

    protected:
        thread* _waiter = CURRENT;
        std::atomic_flag _signaled = ATOMIC_FLAG_INIT;
